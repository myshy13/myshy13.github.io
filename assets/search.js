...
if (searchQuery === null || searchQuery === "") {
  console.info("No search query available, not searching.");
} else {
  console.log("searchQuery", searchQuery);
  console.log("results: " + results);

  const results = filterArray(games, searchQuery.toLowerCase());
  if (searchQuery === null || searchQuery === "") {
    document.getElementsByClassName(
      "content"
    )[0].innerHTML = `<h1 style="color:white;display:block;">no results</h1>`;
  }
  ...
  if ((results.length = 1)) {
    switch (results[0]) {
      ...
      case "pacman":
        ...
        break;
      case "the world's hardest game":
        window.location.assign(
          `${document.location.protocol}//${document.location.hostname}:${
            document.location.port
          }${"/src/hardestgame/index.html"}`
        );
        break;
      case "the worlds hardest game":
        window.location.assign(
          `${document.location.protocol}//${document.location.hostname}:${
            document.location.port
          }${"/src/hardestgame/index.html"}`
